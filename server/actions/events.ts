  ...
  export async function createEvent(unsafeData: z.infer<typeof eventFormSchema>): Promise<void> {
    ...
    try {
        ...
        if (!success || !userId) {
            ...
            if( rowCount === 0) {
                throw new Error("Event not found our user not authorized to update this event.");
            }
    ...
    try {
        ...
        const {rowCount} = await db
            ...
            if( rowCount === 0) {
                throw new Error("Event not found our user not authorized to delete this event.");
            }
    } catch (error: any) {
        throw new Error(`Failed to update event: ${error.message || error}`);
    } finally {
